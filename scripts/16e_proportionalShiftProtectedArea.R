# Measuring proportional overlap between modeled ranges and protected areas
# ~5 min
...
## For present and future rasters, binarize using the Maxent threshold
...
calcROIoverlapPresent = function(x = presentRas,
                                 p = ROI_ras,
                                 r1 = 6,
                                 r2 = 6){
...
#### Overlap as a proportion ----
totalPresentSums = lapply(presentRas,
                          function(x){cellStats(x,
                                                stat=sum)*6*6})
totalPresentDF = data.frame("Species" = names(totalPresentSums),
                            "currentDist" = unlist(totalPresentSums),
                            row.names = NULL)
totalFutureSums = lapply(futureRas,
                         function(x){cellStats(x,
                                               stat=sum)*6*6})
totalFutureDF = data.frame("Species" = names(totalFutureSums),
                           "futureDist" = unlist(totalFutureSums),
                           row.names = NULL) %>%
  separate(Species, into = c("Species","GCM","SSP"),
           sep = "_") %>%
  mutate(SSP = ifelse(SSP == 245, "SSP2-4.5","SSP5-8.5")) %>%
  mutate(GCM = str_replace_all(GCM, pattern = "-", replacement = "."))

totalROISum = cellStats(ROI_ras, stat=sum)*6*6


presentFracDist = mydata %>%
  filter(Time == "Present") %>%
  select(Species, Common,areaKm2) %>%
  left_join(totalPresentDF) %>%
  mutate(fracProtected = areaKm2/currentDist) %>%
  select(Common, fracProtected) %>%
  mutate(SpeciesFac = factor(Common, levels = speciesNamesList))
  
presentFracProt = mydata %>%
  filter(Time == "Present") %>%
  select(Common,areaKm2) %>%
  mutate(fracProtected = areaKm2/totalROISum) %>%
  select(Common, fracProtected) %>%
  mutate(SpeciesFac = factor(Common, levels = speciesNamesList))

futureFracDist = mydata %>%
  filter(Time == "Future") %>%
  select(Species, Common,GCM,SSP,areaKm2) %>%
  left_join(totalFutureDF, by = c("Species","GCM","SSP")) %>%
  mutate(fracProtected = areaKm2/futureDist) %>%
  mutate(SpeciesFac = factor(Common, levels = speciesNamesList))

futureFracProt = mydata %>%
  filter(Time == "Future") %>%
  select(Species, Common,GCM,SSP,areaKm2) %>%
  left_join(totalFutureDF, by = c("Species","GCM","SSP")) %>%
  mutate(fracProtected = areaKm2/totalROISum) %>%
...
#### Plot results ----
fracDistPlot = ggplot() +
  geom_boxplot(data = futureFracDist,
               aes(x = SpeciesFac, y = fracProtected*100,
                   group = paste0(Species, SSP),
                   col = SSP),
               outlier.size = 1) +
  geom_point(data = presentFracDist,
             aes(x = SpeciesFac, y = fracProtected*100),
             shape = 15, size = 2) +
  scale_color_manual("SSP", values = CJsBasics::KellyCols[c(4,8)]) +
  ylab(expression(paste("% of distribution"))) +
  xlab("") +
...
fracProtPlot = ggplot() +
  geom_boxplot(data = futureFracProt,
               aes(x = SpeciesFac, y = fracProtected*100,
                   group = paste0(Species, SSP),
                   col = SSP),
               outlier.size = 1) +
  geom_point(data = presentFracProt,
             aes(x = SpeciesFac, y = fracProtected*100),
             shape = 15, size = 2) +
  scale_color_manual("SSP", values = CJsBasics::KellyCols[c(4,8)]) +
  ylab(expression(paste("% of protected area"))) +
  xlab("") +
  CJsBasics::BasicTheme +
...
protectedAreaPct = cowplot::ggdraw() +
  draw_plot(fracDistPlot + theme(legend.position = "none"),
            x = 0.005, y = 0.575, 
            width = 0.8, height = 0.425) +
  draw_plot(fracProtPlot + theme(legend.position = "none"),
            x = 0, y = 0, 
            width = 0.8, height = 0.575) +
  draw_plot(cowplot::get_legend(fracDistPlot),
            x = 0.8, y = 0.5,
...
#### Save outputs ----
ggsave(filename = "plots/protectedAreaPct.jpg",
       protectedAreaPct,
       width = 6,
       height = 6,
       units = "in",
